...
### ADR-044: `payroll.calculate` otorgado a `COMPANY_OWNER` y `ADMIN`

**Contexto**: la matriz RBAC original de [06-AUTHORIZATION.md](./06-AUTHORIZATION.md) (Fase 2) excluía a `COMPANY_OWNER` y `ADMIN` del permiso `payroll.calculate`, aparentemente modelando una separación de funciones (cálculo a cargo de un rol dedicado de nómina; aprobación/cierre a cargo de la propiedad). Sin embargo, la aplicación no tiene ninguna UI de gestión de usuarios/roles en el MVP (ver [26-PROGRESS.md](./26-PROGRESS.md): "Gestión en runtime de roles/permisos... hoy solo existe seed estático (RoleSeeder/PermissionSeeder), sin ningún endpoint/UI de gestión post-seed") que le permita a una empresa crear y asignar un usuario `PAYROLL_MANAGER`. Esto se descubrió mediante pruebas manuales en vivo sobre el ambiente de staging en la Fase 15: un propietario autorregistrado (`COMPANY_OWNER`, asignado automáticamente por `CreateNewUser`, ver ADR-024) no tenía ninguna forma de calcular su propia nómina.

**Decisión**: otorgar el permiso `payroll.calculate` a los roles `COMPANY_OWNER` y `ADMIN`. Los grants de `PAYROLL_MANAGER`, `SUPERVISOR`, `HR_MANAGER`, `ACCOUNTANT` y `EMPLOYEE` no cambian.

**Motivo**: el modelo comercial de la v1 es "1 cuenta = 1 empresa" sin ninguna función de invitación/equipo (ADR-024). Sin este grant, un propietario autorregistrado nunca puede calcular su propia nómina — un bloqueador real de producto para el mercado objetivo real (el pequeño empresario que administra su propia nómina), no un caso de borde teórico. Es coherente con el precedente ya establecido en ADR-027 (`payroll.close` sin maker-checker): el propietario del producto ya rechazó la fricción de separación de funciones para el tamaño de negocio de la v1.

**Alternativas consideradas**: construir primero una UI completa de invitación de usuarios y asignación de roles (descartada por ser un alcance desproporcionado para resolver este hueco puntual; queda como el hueco ya documentado en [26-PROGRESS.md](./26-PROGRESS.md), a construir cuando la gestión de equipos se convierta en una funcionalidad real); dejar a `COMPANY_OWNER` sin poder calcular su nómina y requerir que el personal de la plataforma (`SUPER_ADMIN`) lo haga en nombre del cliente (descartada: carga operativa y de soporte inaceptable para un uso básico).

**Consecuencias**: la matriz de [06-AUTHORIZATION.md](./06-AUTHORIZATION.md) y `RoleSeeder` quedan actualizados. Los ambientes ya sembrados (staging y producción) necesitan que `RoleSeeder` se vuelva a ejecutar manualmente para recoger el nuevo grant, dado que (ver [26-PROGRESS.md](./26-PROGRESS.md)) los 5 seeders de `DatabaseSeeder` se ejecutan una sola vez por ambiente vía la funcionalidad "Commands" del dashboard de Laravel Cloud, no como parte de los comandos de deploy recurrentes — esto queda como un paso manual pendiente, no algo que este commit pueda ejecutar por sí mismo.
