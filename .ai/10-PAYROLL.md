...
2. **Caso borde — contrato partido a mitad de periodo** (ej. una promoción efectiva el día 8 de una quincena): el esquema debe soportar el prorrateo mediante **múltiples `payroll_entry_lines` con distinto `contract_id`** dentro de la misma `payroll_entry`. Esto ya es una decisión de diseño tomada: el esquema lo permite estructuralmente.
   - **PENDING DECISION**: el algoritmo exacto de prorrateo (por días calendario, por días hábiles, por horas planificadas, u otro criterio) depende de legislación laboral no definida en el brief. Mientras no se resuelva, el motor debe declarar el sub-rango como un `payroll_entry_line` separado por contrato, sin inventar una fórmula de prorrateo.
...
- **Contrato partido a mitad de periodo**: ver regla "Determinación de qué contrato/salario aplica a un periodo dado" arriba. El esquema soporta múltiples `payroll_entry_lines` con distinto `contract_id`; el algoritmo exacto de prorrateo es `PENDING DECISION`.
- **Empleado sin asistencia registrada en el periodo** (ej. ingresó después del cierre de asistencia, o hubo una falla de captura): el motor no debe asumir asistencia completa por defecto. El comportamiento debe ser explícito: o se bloquea el cálculo de ese empleado hasta que exista al menos un `attendance_record` (o una `novelty_record` que justifique la ausencia total, como vacaciones o incapacidad), o se liquida solo con los conceptos no dependientes de asistencia (ej. deducciones fijas), dejando en cero los conceptos que sí dependen de tiempo trabajado. Cuál de estos dos comportamientos aplica no está resuelto por el brief.
  - **PENDING DECISION**: comportamiento exacto del motor ante un empleado sin ningún `attendance_record` ni `novelty_record` que justifique la ausencia dentro del periodo.
