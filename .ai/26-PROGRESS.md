...
- **`work_schedule_days` no tiene columnas de descanso planificado** (ver `PENDING DECISION` agregado en [08-SHIFTS.md](./08-SHIFTS.md)): la generación automática desde plantilla (Fase 5) crea `shifts` + `shift_assignments` pero nunca `shift_breaks`; los turnos partidos y descansos se agregan manualmente sobre un turno ya generado.
