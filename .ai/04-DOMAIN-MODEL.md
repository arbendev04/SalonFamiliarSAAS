...
**PENDING DECISION**: si un empleado tiene una ausencia completa detectada por el motor de Time Calculation **sin ningún `leave_records` que la justifique**, ¿debe generarse igual un `absence_records` con `justified=false`, `leave_record_id=null`, `source='time_calculation'`? El esquema (`leave_record_id` nullable, `source` genérico) sugiere que sí en algún momento, pero ningún criterio de aceptación de la Fase 8 lo exige. Implementado alrededor: la Fase 8 **solo escribe `absence_records` desde el lado `leave_records`-aprobado** (`justified=true`, vía `LeaveRecordService::generateNoveltyAndAbsence()`); el caso no justificado queda representado únicamente en `attendance_records.missing_minutes`, sin fila propia en `absence_records` todavía.
