...
**Descanso planificado a nivel de plantilla**: `work_schedule_days` admite `break_start_time`/`break_end_time` opcionales (ambos presentes o ambos ausentes, validado en `StoreWorkScheduleTemplateRequest::withValidator()`), además de `start_time`/`end_time`/`crosses_midnight`. Cuando una regla de día define ese par, la generación desde plantilla (flujo 3) crea automáticamente el `shift_breaks` correspondiente junto con el `shift`, anclando `planned_start`/`planned_end` a la fecha real del turno con el mismo criterio de `crosses_midnight` que usa `ShiftGenerator` para `start_datetime`/`end_datetime`. Cuando el día de plantilla no define ventana de descanso, la generación se comporta exactamente igual que antes: no crea ningún `shift_breaks`, y este debe agregarse manualmente sobre el turno ya generado (vía el endpoint de `shift_breaks`).
