...
3. **Deduplicación de eventos equivalentes.** Si ya existe un `attendance_event` del mismo empleado, mismo `event_type`, dentro de la misma ventana de tiempo considerada equivalente, el evento entrante se marca `DUPLICATE` y se enlaza al existente — **no se crea un evento nuevo**.
   - **PENDING DECISION**: ¿cuál es la ventana de tiempo exacta (en minutos) usada para considerar dos eventos del mismo empleado/tipo como duplicados? El blueprint establece la regla de deduplicación pero no fija el umbral; probablemente dependa también del proveedor biométrico elegido (ver `PENDING DECISION` de inferencia de tipo de evento más abajo).
